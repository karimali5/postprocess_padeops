...
def _apply_profile_offset(x, y, offset_file):
    if offset_file is None:
        return y

    x_offset, y_offset = load_profile(offset_file)
    x = np.asarray(x)
    x_offset = np.asarray(x_offset)
    if x.shape != x_offset.shape or not np.allclose(x, x_offset):
        raise ValueError(f"Profile x-coordinates do not match offset file {offset_file}.")
    return np.asarray(y) - np.asarray(y_offset)
...
def plot_profiles(profiles, **kwargs):
    ...
    profiles : list of dict
        ...
        'file' and may include 'offset' to subtract another profile before
        applying 'ygain'.
    **kwargs : dict
